    ...
    for (int w=0; w<worker; w++)
    {
        ...
        while (update < worker_update_times)
        {
            ...
            for (int b=0; b<num_negative; b++)
            {
                ...
                // item / profile / meta
                // item 
                pos_collect.push_back( ui_sampler.draw_a_context(user) ); // draw pos_item 1
                // user
                // pos_collect.push_back( iu_sampler.draw_a_context(item) );
                pos_collect.push_back( iu_sampler.draw_a_context(pos_collect[0]) ); // draw pos_ur 1
                pos_collect.push_back( ui_sampler.draw_a_context(pos_collect[1]) ); // draw pos_item 2
