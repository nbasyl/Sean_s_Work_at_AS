...
int main(int argc, char **argv){
    ...
    std::string train_up_path = arg_parser.get_str("-train_user_profile", "/home/syliu/AIR/Data/mvlen_data/user_occupation_graph.train", "input item-meta graph path");
    ...
    FileGraph up_file_graph(train_up_path, 0, iu_file_graph.index2node);
    FileGraph pu_file_graph(train_up_path, -1, up_file_graph.index2node);
    ...
    VCSampler up_sampler(&up_file_graph);
    VCSampler pu_sampler(&pu_file_graph);
    ...
    LookupMapper mapper(pu_sampler.vertex_size, dimension);
    ...
    {
        int num_gradient;
        long user, item, user_profile, pos_i, pos_u;
        ...
        while (update < worker_update_times)
        {
            ...
            for (int b=0; b<num_negative; b++)
            {
                ...
                user_profile = up_sampler.draw_a_context_safely(user);
                if (user_profile != -1)
                {
                    pos_u = pu_sampler.draw_a_context_safely(user_profile);
                    pos_collect.push_back( pos_u );
                    pos_i = ui_sampler.draw_a_context_safely(pos_collect[0]);
                ...
                // std::cout << up_file_graph.index2node[user_profile] << std::endl;
