...
int main(int argc, char **argv){
    ...
    std::string train_im_path = arg_parser.get_str("-train_item_meta", "/home/syliu/AIR/Data/mvlen_data/item_genres_graph.train", "input item-meta graph path");
    ...
    int dimension = arg_parser.get_int("-dimension", 100, "embedding dimension");
    ...
    FileGraph im_file_graph(train_im_path, 0, iu_file_graph.index2node);
    FileGraph mi_file_graph(train_im_path, -1, im_file_graph.index2node);
    // std::vector<FileGraph> uir_file_graph, iur_file_graph;
    // std::vector<FileGraph> im_file_graph, mi_file_graph;
    // std::string sub_path;

    // std::cout << "(UIR-Graph)" << std::endl;
    // std::stringstream ss;
    // ss << train_uir_path;
    // while(ss.good())
    // {
    ...
    //     if (uir_file_graph.size())
    //         uir_file_graph.push_back(FileGraph(sub_path, 0, uir_file_graph.back().index2node));
    //     else
    //         uir_file_graph.push_back(FileGraph(sub_path, 0, ui_file_graph.index2node));
    //     iur_file_graph.push_back(FileGraph(sub_path, -1, uir_file_graph.back().index2node));
    // }
    ...
    VCSampler im_sampler(&im_file_graph);
    VCSampler mi_sampler(&mi_file_graph);

    // std::vector<VCSampler> uir_sampler;
    // for (auto graph: uir_file_graph)
    //     uir_sampler.push_back(VCSampler(&graph));
    // std::vector<VCSampler> iur_sampler;
    // for (auto graph: iur_file_graph)
    //     iur_sampler.push_back(VCSampler(&graph));

    // 2. [Mapper] define what embedding mapper to be used
    LookupMapper mapper(mi_sampler.vertex_size, dimension);
    ...
    {
        int num_gradient;
        long user, item, item_meta, pos_i, pos_u;
        ...
        while (update < worker_update_times)
        {
            ...
            // weight = ui_file_graph.index_graph[user][item];
            std::cout << ui_file_graph.index2node[user] << std::endl;
            std::cout << ui_file_graph.index2node[item] << std::endl;
            ...
            for (int b=0; b<num_negative; b++)
            {
                ...
                item_meta = im_sampler.draw_a_context_safely(item);
                if (item_meta != -1)
                {
                    pos_i = mi_sampler.draw_a_context_safely(item_meta);
                    std::cout << im_file_graph.index2node[pos_i] << std::endl;
                    pos_collect.push_back( pos_i );
                    pos_u = iu_sampler.draw_a_context_safely(pos_collect[0]);
                    std::cout << pos_u << std::endl;
                    pos_collect.push_back( pos_u );
                ...
                // pos_i = uir_sampler[weight-1].draw_a_context_safely(user);
                // if (pos_i != -1)
                //     pos_collect.push_back( pos_i );
                // else
                //     pos_collect.push_back( ui_sampler.draw_a_context(user) );

                // pos_u = iur_sampler[weight-1].draw_a_context_safely(pos_collect[0]);
                // if (pos_i != -1)
                //     pos_collect.push_back( pos_u );
                // else
                //     pos_collect.push_back( iu_sampler.draw_a_context(pos_collect[0]) );
                std::cout << im_file_graph.index2node[item_meta] << std::endl;
                std::cout << ui_file_graph.index2node[pos_collect[0]] << std::endl;
                std::cout << ui_file_graph.index2node[pos_collect[1]] << std::endl;
                // std::cout << ui_file_graph.index_graph[pos_collect[1]][pos_collect[0]] << std::endl;
